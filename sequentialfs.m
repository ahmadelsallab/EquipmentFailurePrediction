...
% $Revision: 1.1.8.6 $  $Date: 2010/10/08 17:26:35 $

if nargin < 2 || isempty(varargin)
    error(message('stats:sequentialfs:TooFewInputs'));
end
...
if ischar(direction)
    ...
    if isempty(i)
        error(message('stats:sequentialfs:UnknownDirection', direction));
    end
    direction = dirNames{i};
else
    error(message('stats:sequentialfs:InvalidDirection'));
end
